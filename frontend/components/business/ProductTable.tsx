...
interface Product {
	...
	images?: string[]; // multiple images (previews)
					...
					<tbody>
						{products.map((p) => (
							...
							<React.Fragment key={p.id}>
								<tr data-id={p.id} className="border-b hover:bg-gray-50 transition-colors">
								...
								<td className="px-4 py-2 text-gray-800 font-medium text-left">
										<div className="flex items-center gap-3">
												{((p.images && p.images.length > 0) ? p.images[0] : p.image) ? (
													<img src={(p.images && p.images.length > 0) ? p.images[0] : p.image} alt={p.name} className="w-12 h-12 object-cover rounded-md bg-white border" />
												) : (
									...
									<td className="px-4 py-2">
										<span
											className={`px-3 py-1 rounded-full text-xs font-medium ${
												(p.status === "ACTIVE" || p.status === "Đang hoạt động" || p.status === "Còn hàng")
													? "bg-green-100 text-green-700"
													: "bg-red-100 text-red-600"
											}`}
										>
											{(p.status === "ACTIVE" || p.status === "Đang hoạt động" || p.status === "Còn hàng") ? 'Đang hoạt động' : 'Ngừng hoạt động'}
										</span>
									</td>
									<td className="px-4 py-4 flex items-center gap-2">
										<Button size="sm" variant="outline" className="flex items-center gap-1" onClick={() => onView?.(p.id)}>
