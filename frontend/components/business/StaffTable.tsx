...
export default function StaffTable({ staff, onEdit }: StaffTableProps) {
  return (
    <div className="bg-gray-50 rounded-xl border p-6">
      <div className="mb-4">
        <h2 className="text-xl font-semibold text-gray-800">Danh sách nhân viên</h2>
        ...
        <table className="min-w-full text-sm border-t border-gray-200 table-fixed">
          ...
          <thead className="bg-gray-100 text-gray-700 font-medium">
            <tr className="border-b">
              <th className="text-left px-4 py-2">Tên tài khoản</th>
              <th className="text-left px-4 py-2">Chức vụ</th>
          ...
          <tbody>
            {staff.map((s) => (
              <tr key={s.MaTK} className="border-b hover:bg-gray-50 transition-colors">
                <td className="px-4 py-3 text-gray-800 font-medium">{s.DisplayName ?? '-'}</td>
                <td className="px-4 py-3 text-gray-600">{s.VAITRO ?? '-'}</td>
                <td className="px-4 py-3 text-gray-600 truncate">{s.Email ?? '-'}</td>
                ...
                <td className="px-4 py-3">
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${s.Status === 'ACTIVE' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                    {s.Status === 'ACTIVE' ? 'Còn hoạt động' : 'Ngừng hoạt động'}
                  </span>
                </td>
                <td className="px-4 py-3">
                  <Button size="sm" variant="outline" className="flex items-center gap-2" onClick={() => onEdit?.(s)}>
