...
interface VariantRow {
	...
	size: string;
	quantity: number;
	detailStatus: string; // 'Hết hàng' | 'Còn hàng' | 'Tạm ngưng'
}
...
export default function ProductViewPopup({ open, onClose, product, focusSizes }: ProductViewPopupProps) {
	...
	useEffect(() => {
		...
		const initialRows: VariantRow[] = normalized.map((sz) => ({
			id: (typeof crypto !== 'undefined' && (crypto as any).randomUUID) ? (crypto as any).randomUUID() : `v-${Date.now()}-${Math.floor(Math.random()*1000)}`,
			size: sz,
			quantity: 0,
		...
		if (product.variants && Array.isArray(product.variants)) {
			for (const r of initialRows) {
				const found = product.variants.find((v: any) => String(v.size) === String(r.size));
				if (found) {
					r.quantity = typeof found.quantity === 'number' ? found.quantity : 0;
					r.detailStatus = (found.detailStatus ?? found.status) ?? (r.quantity > 0 ? 'Còn hàng' : 'Hết hàng');
				}
			}
		}
	...
	function setStatus(index: number, val: string) {
			setRows((prev) => prev.map((r, i) => i === index ? { ...r, detailStatus: val } : r));
		}
		...
		<Dialog open={open} onOpenChange={(o) => { if (!o) onClose(); }}>
			<DialogContent>
				...
				<DialogFooter className="mt-4 flex items-center gap-3">
					<Button variant="outline" onClick={onClose}><X className="w-4 h-4" /> Đóng</Button>
				</DialogFooter>
			</DialogContent>
		</Dialog>
	);
