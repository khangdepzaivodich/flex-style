...
export default function ProductPopup({ open, onClose, onSave, initialData }: ProductPopupProps) {
	...
	useEffect(() => {
		if (open) {
			if (initialData) {
				...
				setSize((initialData as any).size ?? "");
	...
	function validate(): boolean {
		...
		if (!size || !size.trim()) e.size = 'Vui lòng chọn kích thước';
			...
			<DialogContent className="max-h-[90vh] overflow-auto">
						...
						{imagePreview && (
							<div className="mt-3 grid grid-cols-1 gap-3">
								<div className="relative border rounded overflow-hidden">
									<img src={imagePreview} className="w-full h-auto max-h-[50vh] object-contain bg-white" alt={`preview`} />
				...
				<DialogFooter className="justify-center gap-4">
					...
					<Button className="bg-[#8B5CF6] text-white flex items-center gap-2 px-4 py-2 rounded-md" onClick={() => {
						if (!validate()) return;
						onSave({ id, name, category, stock, minStock, price, image: imagePreview ?? undefined, color, size, description });
					}}>
