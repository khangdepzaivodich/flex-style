...
export class PhieuNhapHangService {
    constructor(private readonly phieuNhapHangRepository: PhieuNhapHangRepository) { }
    ...
    async update(id: string, data: PhieuNhapHangDto) {
        const existingPhieuNhapHang = await this.phieuNhapHangRepository.findById(id);
    ...
    async nhanVienXacNhan(id: string, MaTKNVXN: string, TrangThai: string) {
        ...
        if (existingPhieuNhapHang.TrangThai !== 'NCC_XACNHAN') {
            throw new Error('Phiếu nhập hàng phải ở trạng thái nhà cung cấp xác nhận thì nhân viên mới có thể xác nhận');
        }
        return this.phieuNhapHangRepository.nhanVienXacNhan(id, MaTKNVXN, TrangThai as TrangThaiPhieuNhapHang);
    }
    ...
    async nhaCungCapXacNhan(id: string, TrangThai: string) {
        ...
        if (existingPhieuNhapHang.TrangThai !== 'DANG_CHO') {
            throw new Error('Phiếu nhập hàng phải ở trạng thái đang chờ thì nhà cung cấp mới có thể xác nhận');
        }

        return this.phieuNhapHangRepository.nhaCungCapXacNhan(id, TrangThai as TrangThaiPhieuNhapHang);
    }
    ...
    async nhaCungCapTuChoi(id: string, TrangThai: string) {
        ...
        if (existingPhieuNhapHang.TrangThai !== 'DANG_CHO') {
            throw new Error('Phiếu nhập hàng phải ở trạng thái đang chờ thì nhà cung cấp mới có thể từ chối');
        }
        return this.phieuNhapHangRepository.nhaCungCapTuChoi(id, TrangThai as TrangThaiPhieuNhapHang);
    }
}
