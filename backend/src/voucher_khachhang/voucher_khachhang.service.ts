...
export class VoucherKhachHangService {
    ...
    async findAll(MaKH: string) {
        ...
        for (const voucher of vouchers) {
            const checkVoucher = await this.voucherRepository.getVoucherById(voucher.MaVoucher);
            if (checkVoucher) {
                // Nếu voucher tồn tại, kiểm tra trạng thái
                if (checkVoucher.TrangThai === 'INACTIVE') {
                    await this.voucherKhachHangRepository.updateVoucherStatus(voucher.MaVoucher, TrangThai.INACTIVE);
                } else {
                    await this.voucherKhachHangRepository.updateVoucherStatus(voucher.MaVoucher, TrangThai.ACTIVE);
                }
            }
    ...
    async findById(MaVCKH: string) {
        ...
        const voucherDetails = await this.voucherRepository.getVoucherById(voucher.MaVoucher);
        if (voucherDetails) {
    ...
    async add(MaKH: string, MaVoucher: string) {
        ...
        if (checkVoucher.NgayBatDau > currentDate || checkVoucher.NgayKetThuc < currentDate) {
